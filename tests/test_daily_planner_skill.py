...
class DailyPlannerSkillTests(unittest.TestCase):
    ...
    def test_ssot_and_safe_write_contract_is_explicit(self):
        for phrase in (
            ...
            "`Daily/YYYY-MM-DD.md` for captured tasks and saved daily plans",
            "Save explicit daily-plan captures in the current `Daily/YYYY-MM-DD.md` note",
            "Each calendar day has its own note",
            "Never use one shared `Daily.md` file",
            "Do not write captured tasks to `Planner/Inbox.md`",
            "inside the daily note's managed `### Tasks` section",
